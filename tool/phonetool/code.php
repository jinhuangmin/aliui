...
for($i=1;$i<=$d;$i++){
	...
	$popImg=$code[m][$i-1]?',"m":"'.$code[m][$i-1].'"':'';
	$code2.='{"o":'.($i+1).',"t": "A"'.$popImg.',"a": {"data-t": "area","style": {"width": "'.$code[d][$i-1][0].'px","height": "'.$code[d][$i-1][1].'px","left": "'.$code[d][$i-1][2].'px","top": "'.$code[d][$i-1][3].'px","position": "absolute"},"target":"_blank"'.$url.'}},';
}
$num=array();
for($i=1;$i<=$d;$i++){
	$num[$i-1]=$i+1;
